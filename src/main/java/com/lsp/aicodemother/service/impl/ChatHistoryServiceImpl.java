...
import com.lsp.aicodemother.mapper.ChatHistoryMapper;
...
public class ChatHistoryServiceImpl extends ServiceImpl<ChatHistoryMapper, ChatHistory>  implements ChatHistoryService{
    ...
    public int loadChatHistoryToMemory(Long appId, MessageWindowChatMemory chatMemory, int maxCount) {
        try {
            //倒叙，让数据库一次查询得到结果，再内存再进行反转，减少数据库排序开销
    ...
    @Override
    public String exportToMarkdown(Long appId) {
        ThrowUtils.throwIf(appId == null || appId <= 0, ErrorCode.PARAMS_ERROR, "应用ID不能为空");
        ...
        for (ChatHistory history : historyList) {
            if (ChatHistoryMessageTypeEnum.USER.getValue().equals(history.getMessageType())) {
                markdownBuilder.append("### 用户:\n");
                markdownBuilder.append(history.getMessage()).append("\n\n");
            } else if (ChatHistoryMessageTypeEnum.AI.getValue().equals(history.getMessageType())) {
                markdownBuilder.append("### AI:\n");
                markdownBuilder.append(history.getMessage()).append("\n\n");
            }
        }
